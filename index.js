...
/** BEGIN: Export methods for unit testing (DO NOT add this to Segment) */
module.exports = {
  __fetchProjectData,
  __uuidv4,
  onTrack,
};
/** END: Unit testing export */

/**
 * OPTIONAL step to improve performance. Complete each time new Events or Attributes are added.
 * 1. Open Chrome JavaScript Console
 * 2. Run this in the Chrome JavaScript Console: { OPTIMIZELY_DATA: await __fetchProjectData('a1b2c3d4e5f6') }
 * 3. Right click on OPTIMIZELY_DATA key, click "Store as global variable"
 * 4. Wrap temp variable output in Chrome copy() method (e.g. copy(temp1), copy(temp2))
 * 5. Paste value below for each item
 */
const OPTIMIZELY_DATA = null;
  ...
  const optimizelyTrackEventResponse = await fetch(
    'https://logx.optimizely.com/v1/events',
    {
      ...
      body: JSON.stringify({
        ...
        client_version: '0.0.1', // TODO(derek@optimizely.com) Get from package.json
 ...
 * @param {String} sdkKey
 */
async function __fetchProjectData(sdkKey) {
